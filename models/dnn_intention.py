...
class AttentionModule(nn.Module):
    ...
    def forward(self, h_m, h_s):
        ...
        combined = torch.cat(
            (h_c, h_m.squeeze(1)), dim=-1
        )  # (batch_size, hidden_size_spacial * 2)
...
class PedestrianCrossingModel(nn.Module):
    def __init__(self):
        ...
        self.shift_body_pose = ShiftGRUModule(
            input_size=2 * KEYPOINTS_NUM, hidden_size=HIDDEN_SIZE_SPACIAL
        )
        # Bounding Box和Body Pose的GRU
        self.gru_bb_bp = nn.GRU(
            2 * HIDDEN_SIZE_SPACIAL, hidden_size=HIDDEN_SIZE_SPACIAL, batch_first=True
        )
        # 对Vehicle Speed进行嵌入
        ...
        # Plm (Local Motion)
        self.local_motion_module = CNNGRUModule(
            input_channels=3, conv_output_size=256, gru_hidden_size=128
        )

        # Plc (Local Content)
        self.local_content_module = CNNGRUModule(
            input_channels=3, conv_output_size=256, gru_hidden_size=128
        )
        ...
        self.semantic_context_conv3d = nn.Conv3d(
            in_channels=3, out_channels=512, kernel_size=3, padding=1
        )
        self.categorical_depth_conv3d = nn.Conv3d(
            in_channels=3, out_channels=512, kernel_size=3, padding=1
        )
        ...
        self.final_gru = nn.GRU(
            input_size=512 + 256 + 256 + 128, hidden_size=128, batch_first=True
        )
    ...
    def forward(
        ...
        # Pbb 和 Pbp 处理
        pbb_out = self.shift_bounding_box(
            pbb_input
        )  # 输出 shape: (batch, seq_len, 256)
        ...
        fused_local = self.attention_module(
            motion_out, content_out
        )  # 输出 shape: (batch, 128)

        # 语义上下文和深度特征处理
        semantic_context_out = self.flatten(
            F.relu(self.semantic_context_conv3d(semantic_context_input))
        )
        depth_out = self.flatten(F.relu(self.categorical_depth_conv3d(depth_input)))

        # 合并特征
        context_depth_out = F.relu(
            self.fc_context_depth(torch.cat((semantic_context_out, depth_out), dim=-1))
        )

        # 最终特征合并并经过 GRU
        combined_features = torch.cat(
            (pbb_out[:, -1, :], pbp_out[:, -1, :], fused_local, context_depth_out),
            dim=-1,
        )
        gru_out, _ = self.final_gru(
